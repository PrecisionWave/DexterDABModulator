...
| gpsdo_locked | READ ONLY, shows the status locked or unlocked of the GPSDO, the status will get unlocked when the frequency error measured by the PPS pulse is too big, this typically occurs after the PPS is coming back after some hours of absence, but not on shorter interruptions (also see pps_loss_of_signal attribute). The attribute is written by the gpsdo.sh control script | bool |
